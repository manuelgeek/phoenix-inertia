	...
	<div class="w-full h-full max-w-xs self-center">
		...
		<form class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4" @submit.prevent="register">
            ...
            $page.errors.password ? ' border-red-500' : ''
				...
				>{{ $page.errors.password[0] }}</p>
			</div>
			<div class="mb-6">
				<label class="block text-gray-700 text-sm font-bold mb-2" for="password_c">Password Confirm</label>
				<input
					id="password_c"
					v-model="form.password_confirmation"
					:class="[
            'shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 mb-3 leading-tight focus:outline-none focus:shadow-outline ',
            $page.errors.password_confirmation ? ' border-red-500' : ''
          ]"
					type="password"
				...
				<p
					...
					class="text-red-500 text-xs italic"
				>{{ $page.errors.password_confirmation[0] }}</p>
			</div>
